...
const (
	...
	// The source audio is preserved exactly as downloaded.  The two stems are
	// generated by the CUDA Colab separator before STT; only the vocal stem is
	// submitted to transcription and only the background stem is mixed back
	// under the translated voice.
	SubtitleTaskVocalAudioFileName                               = "source_vocals.wav"
	SubtitleTaskBackgroundAudioFileName                          = "source_background.wav"
...
const (
	...
	TtsMixedAudioFileName           = "tts_with_background.wav"
)
...
type SubtitleTaskStepParam struct {
	...
	TaskBasePath string
	Link         string
	// SourceCookieBrowser is only the selected browser engine (auto/chrome/
	// edge/none). It contains no cookie value. KOVA's temporary browser profile
	// exists only for the duration of a retry command.
	SourceCookieBrowser         string
	AudioFilePath               string
	SourceAudioFilePath         string // downloaded, unmodified source audio
	VocalAudioFilePath          string // music-free vocal stem used by STT
	BackgroundAudioFilePath     string // no-vocals stem mixed under dubbed speech
	...
	TtsMixedAudioFilePath       string // dubbed speech + separated source background
	...
	// BlurOriginalText masks a normalized band containing hardcoded source
	// captions before KOVA burns the approved subtitle SRT into the final video.
	BlurOriginalText bool
	BlurRegionX      float64
	BlurRegionY      float64
	BlurRegionWidth  float64
	BlurRegionHeight float64
	BlurStrength     int
	...
	// DubbingProgress is the equivalent observable callback for the audio
	// stage. It is intentionally in-memory; the staged workflow persists the
	// resulting phase rows for desktop polling.
	DubbingProgress func(id string, percent uint8, detail string)
}
