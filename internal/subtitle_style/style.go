...
type Style struct {
	...
	// The fields below are Kova preview/draft metadata. ASS has no separate
	// background/shadow alpha or normalized vertical coordinate, so keeping
	// them explicitly lets a saved Kova preset round-trip without sacrificing
	// the existing ASS-compatible fields above.
	BackgroundAlpha  *float64 `json:"kova_background_alpha,omitempty"`
	ShadowAlpha      *float64 `json:"kova_shadow_alpha,omitempty"`
	VerticalPosition *float64 `json:"kova_vertical_position,omitempty"`
}
...
func mergeStyleRest(base, override Style) Style {
	...
	if override.BackgroundAlpha != nil {
		base.BackgroundAlpha = styleFloatPtr(*override.BackgroundAlpha)
	}
	if override.ShadowAlpha != nil {
		base.ShadowAlpha = styleFloatPtr(*override.ShadowAlpha)
	}
	if override.VerticalPosition != nil {
		base.VerticalPosition = styleFloatPtr(*override.VerticalPosition)
	}
	return base
}
...
func validateStyle(path, expectedName string, style Style) error {
	...
	if err := validateFloat(path+".kova_background_alpha", style.BackgroundAlpha, 0, 1); err != nil {
		return err
	}
	if err := validateFloat(path+".kova_shadow_alpha", style.ShadowAlpha, 0, 1); err != nil {
		return err
	}
	if err := validateFloat(path+".kova_vertical_position", style.VerticalPosition, -1, 1); err != nil {
		return err
	}
