...
func TestAssetNameForCurrentPlatform(t *testing.T) {
	...
	if got != "Kova-cli_2.0.3_macOS_arm64" {
	...
	if got != "Kova-cli_2.0.3_Linux_x86_64" {
		t.Fatalf("asset name = %q", got)
	}
	...
	if got != "Kova-cli_2.0.3_Windows.exe" {
		t.Fatalf("asset name = %q", got)
	}
}
...
func TestResolveAssetFindsMatchingReleaseAsset(t *testing.T) {
	release := Release{
		TagName: "v2.0.3",
		Assets: []Asset{
			{Name: "Kova_2.0.3_macOS_arm64", BrowserDownloadURL: "server"},
			{Name: "Kova-cli_2.0.3_macOS_arm64", BrowserDownloadURL: "cli"},
		},
	}
...
func TestRunDownloadsAndReplacesCurrentExecutable(t *testing.T) {
	dir := t.TempDir()
	target := filepath.Join(dir, "kova-cli")
	...
	runner := &fakeRunner{
		...
		release: Release{
			TagName: "v2.0.3",
			Assets: []Asset{{
				Name:               "Kova-cli_2.0.3_macOS_arm64",
				BrowserDownloadURL: "https://example.test/Kova-cli_2.0.3_macOS_arm64",
			}},
		},
	}
	...
	wantDownloads := []string{"https://example.test/Kova-cli_2.0.3_macOS_arm64"}
...
func TestRunDownloadsWindowsUpdateBesideExecutable(t *testing.T) {
	dir := t.TempDir()
	target := filepath.Join(dir, "kova-cli.exe")
	if err := os.WriteFile(target, []byte("old"), 0755); err != nil {
		t.Fatal(err)
	}
	runner := &fakeRunner{
		...
		release: Release{
			TagName: "v2.0.3",
			Assets: []Asset{{
				Name:               "Kova-cli_2.0.3_Windows.exe",
				BrowserDownloadURL: "https://example.test/Kova-cli_2.0.3_Windows.exe",
			}},
		},
	}
