...
const DefaultRepo = "nqthaivl/Kova"
...
func Run(ctx context.Context, req Request, runner Runner) (Result, error) {
	...
	if !req.Force && sameVersion(req.CurrentVersion, release.TagName) {
		return Result{
			...
			Message: "Kova đã là phiên bản mới nhất / Kova is up to date",
		}, nil
	}
	target := req.Target
	if target == "" {
...
func assetName(version, goos, goarch string) (string, error) {
	...
	switch goos {
	case "darwin":
		return fmt.Sprintf("Kova-cli_%s_macOS_%s", version, goarch), nil
	case "linux":
		return fmt.Sprintf("Kova-cli_%s_Linux_%s", version, releaseArch(goarch)), nil
	case "windows":
		arch := ""
		switch goarch {
		case "amd64", "":
		...
		return fmt.Sprintf("Kova-cli_%s_Windows%s.exe", version, arch), nil
	default:
