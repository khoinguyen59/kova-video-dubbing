...
import (
	...
	"kova/internal/updater"
	"kova/internal/voices"
	"os"
	"path/filepath"
	"runtime"
	"strings"
...
func Help(cmd Command) string {
	...
	case "subtitle":
		return `Usage:
  kova-cli subtitle <input> --origin-lang <lang> --target-lang <lang> --workdir <dir> [flags]
...
`
	case "tts":
		return `Usage:
  kova-cli tts --workdir <dir> --input-srt <file> [flags]
...
`
	case "render-horizontal":
		return `Usage:
  kova-cli render-horizontal --workdir <dir> --video <file> --subtitle <file> [flags]
...
`
	case "render-vertical":
		return `Usage:
  kova-cli render-vertical --workdir <dir> --video <file> --subtitle <file> [flags]
...
`
	case "pipeline":
		return `Usage:
  kova-cli pipeline --outputs <list> [flags]
...
`
	case "cover":
		return `Usage:
  kova-cli cover --workdir <dir> --prompt <text> [flags]
...
`
	case "update":
		return `Usage:
  kova-cli update [flags]

Flags:
  --repo <owner/name>  GitHub repository to update from (default nqthaivl/Kova)
...
`
	case "voices":
		return `Usage:
  kova-cli voices [flags]

Flags:
  --provider <name>  TTS provider: aliyun, openai, minimax, omnivoice, or edge-tts; default current config
...
Commands:
  ...
  update               Update kova-cli from GitHub releases
  voices               List available TTS voice codes
  status               Reserved status query surface

Run "kova-cli <command> --help" for command-specific flags.
`
	}
}
