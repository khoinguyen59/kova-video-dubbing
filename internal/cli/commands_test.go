...
func TestParseUpdateCommand(t *testing.T) {
	...
	if cmd.Update.Repo != "nqthaivl/Kova" {
		t.Fatalf("Repo = %q", cmd.Update.Repo)
	}
	if cmd.Update.Target != "/usr/local/bin/kova-cli" {
...
func TestVoicesHelp(t *testing.T) {
	...
	help := Help(cmd)
	if !strings.Contains(help, "kova-cli voices") || !strings.Contains(help, "--provider") {
...
func TestUpdateHelp(t *testing.T) {
	...
	help := Help(cmd)
	if !strings.Contains(help, "kova-cli update") || !strings.Contains(help, "--version") {
...
func TestExecuteDryRunUpdateReturnsJSONReadyResponse(t *testing.T) {
	...
	if resp.Inputs["repo"] != "nqthaivl/Kova" {
...
func TestExecuteDryRunRenderLoadsSubtitleStyleFile(t *testing.T) {
	...
	manifestPath := filepath.Join(dir, "kova_manifest.json")
