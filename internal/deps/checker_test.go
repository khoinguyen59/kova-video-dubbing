...
func TestFasterWhisperReleaseURLsUsePublicStandaloneAssets(t *testing.T) {
	for _, test := range []struct {
		goos string
		want string
	}{
		{"windows", "Whisper-Faster_r192.3_windows.zip"},
		{"linux", "Whisper-Faster_r189.1_linux.zip"},
	} {
		url, err := fasterWhisperReleaseURL(test.goos)
		if err != nil || !strings.Contains(url, "github.com/Purfview/whisper-standalone-win") || !strings.Contains(url, test.want) {
			t.Fatalf("fasterWhisperReleaseURL(%q) = (%q, %v)", test.goos, url, err)
		}
	}
	if _, err := fasterWhisperReleaseURL("darwin"); err == nil {
		t.Fatal("macOS unexpectedly received a local Faster-Whisper standalone URL")
	}
}
