...
func kovaDependencyURL(asset string) (string, error) {
	baseURL := strings.TrimRight(strings.TrimSpace(os.Getenv("KOVA_DEPENDENCY_BASE_URL")), "/")
	if baseURL == "" {
		return "", fmt.Errorf(
			"dependency %s is missing; install it in PATH/bin or set KOVA_DEPENDENCY_BASE_URL to a Kova-compatible archive mirror",
			asset,
		)
	}
	return baseURL + "/" + asset, nil
}

func CheckDependency() error {
	return checkDependency(true)
}

// CheckPlatformSubtitleDependency prepares only the tools used by a public
// source workflow. It deliberately does not download or require a local ASR
// model: KOVA uses the configured cloud speech-to-text provider after the
...
// engine. Call it after source media has downloaded so its potentially long
// first-run download belongs to the visible speech-to-text phase.
func CheckLocalTranscriptionDependency() error {
	switch config.Conf.Transcribe.Provider {
	case "fasterwhisper":
		return checkFasterWhisper()
	case "whisperkit":
		if err := checkWhisperKit(); err != nil {
			return err
		}
		return checkModel("whisperkit")
	case "whispercpp":
		if err := checkWhispercpp(); err != nil {
			return err
		}
		return checkModel("whispercpp")
	case "openai", "aliyun":
		return nil
	default:
		return fmt.Errorf("unsupported local transcription provider: %s", config.Conf.Transcribe.Provider)
	}
}

func checkDependency(needLocalTranscription bool) error {
	...
	if needLocalTranscription && config.Conf.Transcribe.Provider == "fasterwhisper" {
	...
	if needLocalTranscription && config.Conf.Transcribe.Provider == "whisperkit" {
		if err = checkWhisperKit(); err != nil {
	...
	if needLocalTranscription && config.Conf.Transcribe.Provider == "whisperx" {
		err = checkWhisperX()
		if err != nil {
	...
	if needLocalTranscription && config.Conf.Transcribe.Provider == "whispercpp" {
		if err = checkWhispercpp(); err != nil {
...
func checkAndDownloadFfmpeg() error {
	...
	var ffmpegAsset string
	if runtime.GOOS == "linux" {
		ffmpegAsset = "ffmpeg-6.1-linux-64.zip"
	} else if runtime.GOOS == "darwin" {
		ffmpegAsset = "ffmpeg-6.1-macos-64.zip"
	} else if runtime.GOOS == "windows" {
		ffmpegAsset = "ffmpeg-6.1-win-64.zip"
	} else {
		log.GetLogger().Error("不支持你当前的操作系统", zap.String("当前系统", runtime.GOOS))
		return fmt.Errorf("unsupported OS: %s", runtime.GOOS)
	}
	ffmpegURL, err := kovaDependencyURL(ffmpegAsset)
	if err != nil {
		return err
	}
...
func checkAndDownloadFfprobe() error {
	...
	var ffprobeAsset string
	if runtime.GOOS == "linux" {
		ffprobeAsset = "ffprobe-6.1-linux-64.zip"
	} else if runtime.GOOS == "darwin" {
		ffprobeAsset = "ffprobe-6.1-macos-64.zip"
	} else if runtime.GOOS == "windows" {
		ffprobeAsset = "ffprobe-6.1-win-64.zip"
	} else {
		log.GetLogger().Error("不支持你当前的操作系统", zap.String("当前系统", runtime.GOOS))
		return fmt.Errorf("unsupported OS: %s", runtime.GOOS)
	}
	ffprobeURL, err := kovaDependencyURL(ffprobeAsset)
	if err != nil {
		return err
	}
...
func checkFasterWhisper() error {
	return ensurePublicFasterWhisper()
}

// checkFasterWhisperLegacy retains the historic private-mirror installer for
// reference only. New KOVA runs always use ensurePublicFasterWhisper above.
func checkFasterWhisperLegacy() error {
	var (
		filePath string
		err      error
	)
	...
	if _, err = os.Stat(filePath); os.IsNotExist(err) {
		...
		var dependencyAsset string
		if runtime.GOOS == "windows" {
			dependencyAsset = "Faster-Whisper-XXL_r194.5_windows.zip"
		} else {
			dependencyAsset = "Faster-Whisper-XXL_r192.3.1_linux.zip"
		}
		downloadUrl, err := kovaDependencyURL(dependencyAsset)
		if err != nil {
			return err
		}
...
// 检测whisperkit
// ensurePublicFasterWhisper uses the public standalone release rather than a
// private mirror. Its model directory is passed to the executable; the
// standalone engine downloads the selected model on first transcription.
func ensurePublicFasterWhisper() error {
	const installRoot = "./bin/faster-whisper"
	if filePath, err := findFasterWhisperBinary(installRoot); err == nil {
		if runtime.GOOS != "windows" {
	...
	downloadURL, err := fasterWhisperReleaseURL(runtime.GOOS)
	if err != nil {
		return err
	}
	...
	archivePath := "./bin/faster-whisper.zip"
	if err := util.DownloadFile(downloadURL, archivePath, config.Conf.App.Proxy); err != nil {
		return fmt.Errorf("download Faster-Whisper: %w", err)
	}
	if err := util.Unzip(archivePath, installRoot); err != nil {
		return fmt.Errorf("extract Faster-Whisper: %w", err)
	}
	filePath, err := findFasterWhisperBinary(installRoot)
	if err != nil {
		return fmt.Errorf("Faster-Whisper executable was not found after extraction: %w", err)
	}
	if runtime.GOOS != "windows" {
		if err := os.Chmod(filePath, 0755); err != nil {
			return err
...
func fasterWhisperReleaseURL(goos string) (string, error) {
	switch goos {
	case "windows":
		return "https://github.com/Purfview/whisper-standalone-win/releases/download/faster-whisper/Whisper-Faster_r192.3_windows.zip", nil
	case "linux":
		return "https://github.com/Purfview/whisper-standalone-win/releases/download/faster-whisper/Whisper-Faster_r189.1_linux.zip", nil
	default:
		return "", fmt.Errorf("Faster-Whisper local STT is unsupported on %s", goos)
	}
}

func findFasterWhisperBinary(root string) (string, error) {
	var found string
	err := filepath.Walk(root, func(path string, info os.FileInfo, walkErr error) error {
		...
		if name == "whisper-faster.exe" || name == "whisper-faster" || name == "faster-whisper-xxl.exe" || name == "faster-whisper-xxl" {
			found = path
		}
		return nil
	})
...
func checkWhispercpp() error {
	...
	if _, err = os.Stat(filePath); os.IsNotExist(err) {
		...
		if runtime.GOOS == "windows" {
			downloadUrl, err = kovaDependencyURL("whispercpp-windows-cuda.zip")
			if err != nil {
				return err
			}
		}
...
func checkModel(whisperType string) error {
	...
	case "fasterwhisper":
		...
		if _, err = os.Stat(modelPath); os.IsNotExist(err) {
			...
			downloadUrl, err := kovaDependencyURL(fmt.Sprintf("faster-whisper-%s.zip", model))
			if err != nil {
				return err
			}
	...
	case "whisperkit":
		...
		if len(files) == 0 {
			...
			downloadUrl, err := kovaDependencyURL("whisperkit-large-v2.zip")
			if err != nil {
				return err
			}
