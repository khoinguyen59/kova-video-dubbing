...
var (
	protectedTermTokenPattern = regexp.MustCompile(`\[\[KOVA_PROPER_\d+\]\]`)
...
func (t *Translator) SplitTextAndTranslate(inputText string, originLang, targetLang types.StandardLanguageCode) ([]*TranslatedItem, error) {
	...
	for i, sentence := range sentences {
		...
		go func(index int, originText string) {
			...
			if err != nil {
				select {
				case errChan <- fmt.Errorf("translate subtitle %d: %w", index+1, err):
				default:
				}
				return
			}

			translatedText, err = t.enforceTargetLanguage(originText, translatedText, originLang, targetLang)
			if err != nil {
				select {
				case errChan <- fmt.Errorf("verify subtitle %d: %w", index+1, err):
				default:
				}
				return
			}
...
func (t *Translator) isTranslationValid(originText, translatedText string, originLang, targetLang types.StandardLanguageCode) bool {
	...
	if strings.TrimSpace(originText) == strings.TrimSpace(translatedText) {
		// A platform may label a Vietnamese VTT as English. Preserve an already
		// Vietnamese cue when Vietnamese is also the requested target. Still scan
		// that cue for genuine residual English; otherwise a mixed VTT line could
		// bypass the target-language gate merely because source and target match.
		if targetLang == types.LanguageNameVietnamese && containsVietnameseMarker(originText) {
			return !needsVietnameseReview(originText, translatedText)
		}
		// 检查是否是专有名词、数字或特殊符号
	...
	// Vietnamese is the active dubbing target. Reject an ordinary English word
	// here so retries and the final quality gate can repair it instead of letting
	// it reach subtitles or TTS unchanged.
	if targetLang == types.LanguageNameVietnamese && needsVietnameseReview(originText, translatedText) {
		return false
	}
...
func needsVietnameseReview(origin, translated string) bool {
	// Stable proper-name tokens are deliberately left intact while sending text
	// through an LLM. They are not ordinary English and must not cause the
	// Vietnamese quality gate to "translate" or corrupt a protected name.
	origin = protectedTermTokenPattern.ReplaceAllString(origin, "")
	translated = protectedTermTokenPattern.ReplaceAllString(translated, "")
	if strings.TrimSpace(origin) == "" {
		return false
	}
	// A YouTube VTT can be labelled English even though its actual cue is already
	// Vietnamese. Do not judge it from metadata or from a shared Latin token.
	// Instead, inspect only candidate tokens that are clearly ordinary English.
	// This permits Vietnamese text containing normal ASCII loanwords such as
	// "video", while still rejecting a mixed cue such as "Hãy watch video này".
	if containsVietnameseMarker(translated) {
		return containsResidualEnglishWord(origin, translated)
	}

	targetTokens := latinWords(translated)
	if len(targetTokens) == 0 {
		return false
	}
	if containsVietnameseMarker(origin) {
		// The candidate has no Vietnamese characters although the source does.
		// It may still be a standalone proper name or acronym, but ordinary Latin
		// words here are a failed translation.
		return hasOrdinaryLatinToken(targetTokens)
	}

	// A non-Vietnamese source and a wholly Latin candidate is a failed
	// Vietnamese translation unless the candidate is just an approved name,
	// brand, acronym, code, URL, or a common Vietnamese loanword.
	if hasOrdinaryLatinToken(targetTokens) {
		return true
	}
	return false
}
...
func containsResidualEnglishWord(origin, translated string) bool {
	...
	for _, token := range latinWords(translated) {
		...
		if _, ambiguousVietnameseSyllable := vietnameseAmbiguousASCIISyllables[lower]; ambiguousVietnameseSyllable {
			// A few ordinary Vietnamese words are written entirely in ASCII yet
			// overlap with English (for example "lý do" and "so sánh"). Keep
			// them only in a recognisable Vietnamese collocation. Otherwise, a
			// carried-over English word such as "Hãy do this" remains rejected.
			if hasVietnameseASCIISyllableContext(translated, lower) {
				continue
			}
		...
		if isAllowedVietnameseLatinToken(token) {
			continue
		}
		if _, isEnglish := ordinaryEnglishWords[lower]; isEnglish {
			return true
		...
		// content than a proper noun. Names/brands should retain capitalization or
...
func hasOrdinaryLatinToken(tokens []string) bool {
...
// latinWords returns standalone ASCII words only. A regex alone would extract
// the "t" and "i" from "tôi", causing one-letter English checks to reject
// perfectly valid Vietnamese subtitles.
func latinWords(text string) []string {
	...
	for i := 0; i < len(runes); {
		...
		start := i
		for i < len(runes) && (isASCIILetter(runes[i]) || runes[i] == '\'' || runes[i] == '-') {
			i++
		}
		end := i
		if (start > 0 && unicode.IsLetter(runes[start-1])) || (end < len(runes) && unicode.IsLetter(runes[end])) {
			continue
		}
...
func isLowerCaseLatinToken(token string) bool {
	for _, r := range token {
		if isASCIILetter(r) && r >= 'A' && r <= 'Z' {
			return false
		}
	}
	return true
...
func containsVietnameseMarker(text string) bool {
	return strings.ContainsAny(strings.ToLower(text), "ăâđêôơưáàảãạấầẩẫậắằẳẵặéèẻẽẹếềểễệíìỉĩịóòỏõọốồổỗộớờởỡợúùủũụứừửữựýỳỷỹỵ")
}

func isAllowedVietnameseLatinToken(token string) bool {
	lower := strings.ToLower(token)
	if _, ok := vietnameseLatinLoanwords[lower]; ok {
		return true
	}
	if isAllUpperAcronym(token) {
		return true
	}
	// Capitalized, non-English tokens are normally person, product, or place
	// names. Common English sentence starters are checked first, so "Welcome"
	// and "This" are still rejected.
	if startsWithUpperASCII(token) {
		if _, commonEnglish := ordinaryEnglishWords[lower]; !commonEnglish {
			return true
		}
	}
	return false
...
func isAllUpperAcronym(token string) bool {
	letters := 0
	for _, r := range token {
		if !isASCIILetter(r) {
			return false
...
func startsWithUpperASCII(token string) bool {
	for _, r := range token {
		return r >= 'A' && r <= 'Z'
	}
	return false
...
var vietnameseLatinLoanwords = map[string]struct{}{
	"api": {}, "app": {}, "chatgpt": {}, "clip": {}, "email": {}, "github": {},
	"google": {}, "http": {}, "https": {}, "livestream": {}, "openai": {}, "podcast": {},
	"tiktok": {}, "url": {}, "video": {}, "vlog": {}, "web": {}, "website": {}, "wifi": {},
	"youtube": {},
}
...
// residual English in a Vietnamese cue without treating arbitrary Vietnamese
// ASCII syllables (for example "xem" or "ban") as English.
var ordinaryEnglishWords = map[string]struct{}{
	"a": {}, "about": {}, "after": {}, "all": {}, "also": {}, "an": {}, "and": {}, "are": {}, "as": {}, "at": {},
	"be": {}, "because": {}, "become": {}, "beverage": {}, "before": {}, "best": {}, "both": {}, "but": {}, "by": {},
	"can": {}, "check": {}, "choose": {}, "click": {}, "come": {}, "create": {}, "day": {}, "do": {}, "does": {}, "done": {},
	"drink": {}, "each": {}, "english": {}, "example": {}, "explain": {}, "first": {}, "for": {}, "from": {}, "get": {}, "give": {},
	"go": {}, "good": {}, "great": {}, "guide": {}, "have": {}, "hello": {}, "help": {}, "here": {}, "how": {}, "i": {},
	"if": {}, "in": {}, "into": {}, "is": {}, "it": {}, "just": {}, "know": {}, "learn": {}, "like": {}, "make": {},
	"more": {}, "morning": {}, "most": {}, "my": {}, "new": {}, "next": {}, "not": {}, "now": {}, "of": {}, "on": {},
	"one": {}, "only": {}, "or": {}, "our": {}, "out": {}, "please": {}, "right": {}, "run": {}, "see": {}, "set": {},
	"share": {}, "show": {}, "simple": {}, "so": {}, "start": {}, "step": {}, "that": {}, "the": {}, "these": {}, "this": {},
	"to": {}, "today": {}, "translate": {}, "tutorial": {}, "two": {}, "up": {}, "use": {}, "very": {}, "want": {}, "watch": {},
	"we": {}, "welcome": {}, "were": {}, "what": {}, "when": {}, "where": {}, "which": {}, "will": {}, "with": {}, "work": {},
	"would": {}, "you": {}, "your": {},
}
	...
	for batchIdx, batch := range batches {
		...
		if err != nil {
			...
			for _, block := range batch {
				...
				if verifyErr != nil {
					return fmt.Errorf("subtitle %d failed target-language verification: %w", block.Index, verifyErr)
				}
				block.TargetLanguageSentence = verified
			}
		...
		for j, translation := range translations {
			if j < len(batch) {
				verified, verifyErr := t.enforceTargetLanguage(batch[j].OriginLanguageSentence, translation, originLangCode, targetLangCode)
				if verifyErr != nil {
					return fmt.Errorf("subtitle %d failed target-language verification: %w", batch[j].Index, verifyErr)
				}
				batch[j].TargetLanguageSentence = verified
			}
		}
...
func (t *Translator) batchTranslateTexts(texts []string, originLang, targetLang types.StandardLanguageCode) ([]string, error) {
	...
	targetRule := ""
	if targetLang == types.LanguageNameVietnamese {
		targetRule = "11. For Vietnamese output, translate every ordinary English word into natural Vietnamese. Keep Latin text only when it is truly a proper name, brand, acronym, code, URL, or number."
	}
...
10. Copy any token formatted [[KOVA_PROPER_###]] exactly unchanged; it is an approved proper name and must not be translated, split, or removed.
%s
...
{"translations":[{"index":1,"text":"译文1"},{"index":2,"text":"译文2"}]}`,
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if err := json.Unmarshal([]byte(cleanResponse), &result); err != nil {
			lastErr = fmt.Errorf("không thể phân tích JSON phản hồi: %w", err)
			if attempt == 0 {
				// 只在第一次失败时记录完整响应
		...
		if len(result.Translations) != len(texts) {
			lastErr = fmt.Errorf("số lượng câu dịch không khớp: cần %d, nhận %d", len(texts), len(result.Translations))
	...
	return nil, fmt.Errorf("dịch theo lô thất bại sau %d lần thử: %w", maxAttempts, lastErr)
}
...
func (t *Translator) translateSingleText(text string, originLang, targetLang types.StandardLanguageCode) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("dịch câu thất bại: %w", err)
	}
