...
func (p *Planner) Plan(cues []Cue, language types.StandardLanguageCode) ([]PlanItem, []Chunk, error) {
	...
	for i, cue := range cues {
		...
		if p.cfg.EnableTextRewrite && estimate > available && p.optimizer != nil {
			...
			var err error
			if languageAware, ok := p.optimizer.(LanguageAwareTextOptimizer); ok {
				optimized, err = languageAware.OptimizeForLanguage(context.Background(), clean, available, "estimated_too_long", language)
			} else {
