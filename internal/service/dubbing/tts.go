...
func GenerateRawSegments(ctx context.Context, tts types.Ttser, plan []PlanItem, voice, dir string, run CommandRunner, duration DurationProbe) ([]PlanItem, error) {
	...
	for i := range plan {
		...
		if IsSilenceOnlyText(plan[i].SpokenText) {
			...
			duration := plan[i].OriginalEnd - plan[i].OriginalStart
			if err := retryTTSWithDuration(tts, plan[i].SpokenText, voice, output, duration, 3); err != nil {
...
func retryTTS(tts types.Ttser, text, voice, output string, attempts int) error {
	return retryTTSWithDuration(tts, text, voice, output, 0, attempts)
}
...
func retryTTSWithDuration(tts types.Ttser, text, voice, output string, duration float64, attempts int) error {
	...
	for i := 0; i < attempts; i++ {
		...
		if timed, ok := tts.(DurationAwareTTS); ok && duration > 0 {
			last = timed.Text2SpeechWithDuration(text, voice, output, duration)
		} else {
