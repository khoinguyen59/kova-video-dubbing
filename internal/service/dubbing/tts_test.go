...
type durationAwareFakeTTS struct {
	fakeTTS
	durations []float64
}
...
func TestGenerateRawSegmentsPassesSubtitleSlotToDurationAwareTTS(t *testing.T) {
	dir := t.TempDir()
	tts := &durationAwareFakeTTS{fakeTTS: fakeTTS{writeOnReturn: true}}
	plan := []PlanItem{{Index: 1, OriginalStart: 2, OriginalEnd: 3.75, SpokenText: "Xin chào"}}
	if _, err := GenerateRawSegments(context.Background(), tts, plan, "voice", dir, nil, func(string) (float64, error) {
		return 1.75, nil
	}); err != nil {
		t.Fatalf("GenerateRawSegments() error = %v", err)
	}
	if len(tts.durations) != 1 || tts.durations[0] != 1.75 {
		t.Fatalf("durations = %+v, want [1.75]", tts.durations)
	}
}
