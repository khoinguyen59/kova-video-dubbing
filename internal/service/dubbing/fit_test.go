...
func TestFitTimelineNormalizesZeroSpeedConfig(t *testing.T) {
	...
	if got[0].SpeedFactor != defaults.SpeedMax {
		t.Fatalf("SpeedFactor = %v, want normalized max speed %v", got[0].SpeedFactor, defaults.SpeedMax)
	}
...
func TestFitCueTimelinePreservesOriginalSubtitleSlots(t *testing.T) {
	cfg := DefaultConfig()
	plan := []PlanItem{
		{Index: 1, OriginalStart: 1, OriginalEnd: 2, ActualDuration: 0.9},
		{Index: 2, OriginalStart: 3, OriginalEnd: 4.5, ActualDuration: 1.5},
	}
	got, report, err := FitCueTimeline(plan, cfg)
	if err != nil {
		t.Fatalf("FitCueTimeline() error = %v", err)
	}
	if got[0].NewStart != 1 || got[0].NewEnd != 2 || got[1].NewStart != 3 || got[1].NewEnd != 4.5 {
		t.Fatalf("slots changed: %+v", got)
	}
	if report.MaxSpeedFactor <= 0 {
		t.Fatalf("report = %+v, want measured speed", report)
	}
}

func TestFitCueTimelinePreservesNaturalMinimumSpeedAndPadsShortCue(t *testing.T) {
	cfg := DefaultConfig()
	plan := []PlanItem{
		{Index: 1, OriginalStart: 0, OriginalEnd: 2.4, ActualDuration: 1.36},
	}
	got, report, err := FitCueTimeline(plan, cfg)
	if err != nil {
		t.Fatalf("FitCueTimeline() error = %v", err)
	}
	if math.Abs(got[0].SpeedFactor-cfg.SpeedMin) > 0.000001 {
		t.Fatalf("SpeedFactor = %.6f, want natural minimum %.6f", got[0].SpeedFactor, cfg.SpeedMin)
	}
	if !warningsContain(report.Warnings, "padding the remaining slot with silence") {
		t.Fatalf("warnings = %+v, want padding warning", report.Warnings)
	}
}

func TestFitCueTimelineUsesExactRequiredSpeedForLongCue(t *testing.T) {
	cfg := DefaultConfig()
	plan := []PlanItem{
		{Index: 1, OriginalStart: 0, OriginalEnd: 1, ActualDuration: 1.6},
	}
	got, report, err := FitCueTimeline(plan, cfg)
	if err != nil {
		t.Fatalf("FitCueTimeline() error = %v", err)
	}
	if math.Abs(got[0].SpeedFactor-1.6) > 0.000001 {
		t.Fatalf("SpeedFactor = %.6f, want exact required speed 1.6 to avoid clipping", got[0].SpeedFactor)
	}
	if !warningsContain(report.Warnings, "above configured max") {
		t.Fatalf("warnings = %+v, want max-speed warning", report.Warnings)
	}
	if !report.RequiresAttention || len(report.OverLimitIndexes) != 1 || report.OverLimitIndexes[0] != 1 {
		t.Fatalf("report = %+v, want explicit review signal for cue 1", report)
	}
}
