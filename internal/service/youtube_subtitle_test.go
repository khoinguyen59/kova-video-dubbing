...
func TestFindDownloadedSubtitleFileAcceptsAnyTrackForAutoLanguage(t *testing.T) {
	base := t.TempDir()
	path := filepath.Join(base, "video123.en.vtt")
	if err := os.WriteFile(path, []byte("WEBVTT\n"), 0644); err != nil {
		t.Fatal(err)
	}
	got, err := NewYouTubeSubtitleService().findDownloadedSubtitleFile(base, "all", "video123")
	if err != nil {
		t.Fatal(err)
	}
	if got != path {
		t.Fatalf("subtitle path = %q, want %q", got, path)
	}
}
...
func skipYouTubeSubtitleIntegrationTest(t *testing.T) {
	t.Helper()
	if os.Getenv("KOVA_RUN_YOUTUBE_SUBTITLE_INTEGRATION") != "1" {
		t.Skip("set KOVA_RUN_YOUTUBE_SUBTITLE_INTEGRATION=1 to run YouTube subtitle integration tests")
	}
}
