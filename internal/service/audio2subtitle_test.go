...
func Test_splitOriginLongSentence(t *testing.T) {
	...
	s := Service{ChatCompleter: staticChatCompleter{response: `{"short_sentences":[{"text":"then one more thing is search for file count file explorer."},{"text":"Once enabled, nested folder file counts are visible."}]}`}}
	// 执行测试
...
type staticChatCompleter struct {
	response string
}

func (c staticChatCompleter) ChatCompletion(string) (string, error) {
