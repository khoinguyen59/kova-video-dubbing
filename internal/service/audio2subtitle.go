...
func (s Service) transcribeAudio(id int, audioFilePath string, language string, taskBasePath string) (transcriptionData *types.TranscriptionData, err error) {
	...
	if language == "auto" || language == "none" {
		// OpenAI-compatible transcription APIs use an omitted language to
		// trigger detection; the literal value "auto" is not a valid ISO code.
		language = ""
...
func (s Service) processAudioSegments(ctx context.Context, stepParam *types.SubtitleTaskStepParam, timePoints []float64) ([]AudioSegment, error) {
	if len(timePoints) < 2 {
		return nil, fmt.Errorf("audio split requires at least two time points, got %d", len(timePoints))
	}
	for i, timePoint := range timePoints {
		if timePoint < 0 || math.IsNaN(timePoint) || math.IsInf(timePoint, 0) {
			return nil, fmt.Errorf("audio split time point at index %d must be finite and non-negative: %v", i, timePoint)
		}
		if i > 0 && timePoint <= timePoints[i-1] {
			return nil, fmt.Errorf("audio split time points must be strictly increasing at index %d: %v <= %v", i, timePoints[i], timePoints[i-1])
		}
	}
		...
		for {
			...
			case translateItem, ok := <-pendingTranslationQueue:
				...
				for range config.Conf.App.TranslateMaxAttempts {
					...
					if err == nil {
						restoreProtectedTerms(translatedResults, stepParam.ProtectedTerms)
						break
					}
				}
				if err != nil {
		...
		for {
			...
			case transcribedItem := <-transcribedQueue:
				...
				pendingTranslationQueue <- DataWithId[string]{
					Data: protectTerms(transcribedItem.Data.Text, stepParam.ProtectedTerms),
...
func splitSrt(stepParam *types.SubtitleTaskStepParam) error {
	...
	subtitleInfo := types.SubtitleFileInfo{
		...
		Name:               "Phụ đề gốc / " + types.GetStandardLanguageName(stepParam.OriginLanguage) + " Subtitle",
	}
	...
	if stepParam.SubtitleResultType == types.SubtitleResultTypeTargetOnly || stepParam.SubtitleResultType == types.SubtitleResultTypeBilingualTranslationOnBottom || stepParam.SubtitleResultType == types.SubtitleResultTypeBilingualTranslationOnTop {
		subtitleInfo = types.SubtitleFileInfo{
			...
			Name:               "Phụ đề đã dịch / " + types.GetStandardLanguageName(stepParam.TargetLanguage) + " Subtitle",
		}
		if stepParam.UserUILanguage == types.LanguageNameEnglish {
	...
	if stepParam.SubtitleResultType == types.SubtitleResultTypeBilingualTranslationOnTop || stepParam.SubtitleResultType == types.SubtitleResultTypeBilingualTranslationOnBottom {
		subtitleInfo = types.SubtitleFileInfo{
			...
			Name:               "Phụ đề song ngữ / Bilingual Subtitle",
		}
		if stepParam.UserUILanguage == types.LanguageNameEnglish {
