...
func GetSplitPoints(input string, segmentDuration float64) ([]float64, error) {
	if segmentDuration < MIN_SEGMENT_DURATION || math.IsNaN(segmentDuration) || math.IsInf(segmentDuration, 0) {
		return nil, fmt.Errorf("segment duration must be finite and at least %v seconds", MIN_SEGMENT_DURATION)
	}
	...
	if err := validateAudioDuration(audioDuration); err != nil {
		return nil, err
	}
	...
	return finalizeSplitPoints(timePoints, segmentNum, audioDuration), nil
}

func validateAudioDuration(audioDuration float64) error {
	if audioDuration <= 0 || math.IsNaN(audioDuration) || math.IsInf(audioDuration, 0) {
		return fmt.Errorf("audio duration must be finite and greater than zero: %v", audioDuration)
	}
	return nil
}

func finalizeSplitPoints(timePoints []float64, segmentNum int, audioDuration float64) []float64 {
	// If the final segment is too short, merge it into the previous segment.
	// A single-segment clip has no previous segment, so it must remain [0, duration].
