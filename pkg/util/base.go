...
func GetYouTubeID(youtubeURL string) (string, error) {
	...
	if strings.EqualFold(strings.TrimPrefix(parsedURL.Host, "www."), "youtu.be") {
		id := strings.Trim(strings.TrimSpace(parsedURL.Path), "/")
		if id == "" {
	...
	if strings.Contains(parsedURL.Path, "watch") {
		...
		if len(pathSegments) > 0 && strings.TrimSpace(pathSegments[len(pathSegments)-1]) != "" {
...
// IsYouTubeURL accepts the canonical, mobile, short-link, and embed YouTube
// domains. Keeping this in util prevents desktop and service paths from
// disagreeing about a URL such as https://youtu.be/<id>.
func IsYouTubeURL(value string) bool {
	parsedURL, err := url.Parse(strings.TrimSpace(value))
	if err != nil {
		return false
	}
	host := strings.ToLower(strings.TrimPrefix(parsedURL.Hostname(), "www."))
